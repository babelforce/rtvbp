...
fn docs_emitter_projects_the_catalog_roles_examples_and_envelope() {
    ...
    assert!(
        Target::Docs.owns_output_path(Path::new("babelforce.v1/flows/initialize-updated-dtmf.mdx"))
    );
    ...
    let initialization = generated_text(&first, "babelforce.v1/flows/initialize-updated-dtmf.mdx");
    assert!(initialization.contains("Code generated by rtvbp-spec-gen"));
    assert!(initialization.contains("```mermaid"));
    assert!(initialization.contains("participant voice as Voice peer"));
    assert!(initialization.contains("participant application as Application peer"));
    assert!(initialization.contains("REQ session.initialize"));
    assert!(initialization.contains("RES session.initialize"));
    assert!(initialization.contains("EVT session.updated"));
    assert!(initialization.contains("../operations/session.initialize.mdx"));
    assert!(initialization.contains("../events/session.updated.mdx"));

    let barge_in = generated_text(&first, "babelforce.v1/flows/barge-in.mdx");
    assert!(barge_in.contains("EVT audio.speech.started"));
    assert!(barge_in.contains("REQ audio.buffer.clear"));
    assert!(barge_in.contains("RES audio.buffer.clear"));

    let demo_flow = generated_text(&first, "demo.v1/flows/echo-observed.mdx");
    assert!(demo_flow.contains("REQ demo.echo"));
    assert!(demo_flow.contains("EVT demo.observed"));
...
fn vector_emitter_projects_payloads_envelope_cases_and_typed_scenarios() {
    ...
    assert_eq!(
        initialize["cases"][0]["description"],
