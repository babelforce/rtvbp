...
mod profiles;
pub use profiles::{
    ProfileEmitError, emit_go_profiles, emit_profile_docs, emit_profile_manifest,
    emit_profile_vectors, emit_rust_profiles, emit_typescript_profiles,
};
...
impl Target {
    ...
    pub const fn canonical_out_dir(self) -> &'static str {
        match self {
            ...
            Self::TypeScript => "sdk/typescript",
        }
    }
    ...
    pub fn owns_output_path(self, path: &std::path::Path) -> bool {
        match self {
            Self::Manifest => path
                ...
                .is_some_and(|name| {
                    name.ends_with(".catalog.json") || name.ends_with(".profiles.json")
                }),
            Self::Go => {
                let tree = path.iter().next().and_then(|part| part.to_str());
                matches!(tree, Some("catalog" | "envelope" | "profile"))
                    && path
                        .file_name()
                        .and_then(|name| name.to_str())
                        .is_some_and(|name| {
            ...
            Self::Vectors => {
                ...
                matches!(
                    ...
                    || matches!(parts.as_slice(), ["profiles", "negotiation.json"])
            }
            Self::TypeScript => {
                let parts = path
                    .iter()
...
#[derive(Debug, Error)]
#[error("unknown emit target {0:?}; expected manifest, go, rust, typescript, docs, or vectors")]
