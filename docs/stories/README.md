...
`babelforce.v1` wire fixtures, R-2 established the model workspace and imported Go SDK, and R-3
modeled the complete typed catalog. R-4, R-5, and R-8 are now independently ready; everything after
them is sequenced by its `note:`. As each story lands, promote its successor from `backlog` to
`ready`.
...
- [R-4 — Prove spec byte-equality against the golden fixtures](R-4-prove-spec-byte-equality.md) · Proof · frozen fixtures and the typed catalog are ready; prove complete bidirectional byte equality
- [R-5 — Generator skeleton and manifest emitter](R-5-generator-skeleton-manifest.md) · Generator · can run alongside R-4; the cheapest emitter forces the model to be complete
