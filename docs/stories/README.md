...
`babelforce.v1` wire fixtures and R-2 has established the Rust model workspace plus imported Go SDK.
R-3 and R-8 are now independently ready; everything after them is sequenced by its `note:`. As each
story lands, promote its successor from `backlog` to `ready`.
...
- [R-3 — Model babelforce.v1 in the spec crate](R-3-model-babelforce-v1.md) · Spec · next spec story; the model workspace and classic.v1 reference codec are ready
...
- [R-8 — Go runtime core — frame, envelope and transport interfaces plus the memory transport](R-8-runtime-core-interfaces.md) · SDK · can run alongside R-3; defines the seams that WebRTC, QUIC and SIP must later fit
