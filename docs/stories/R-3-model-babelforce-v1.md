---
id: R-3
title: Model babelforce.v1 in the spec crate
pillar: Spec
status: done
priority: 3
design: docs/designs/spec-catalog.md
epic: spec-catalog
areas: [spec]
note: typed payload catalog and validated canonical examples now feed R-4 and R-5
---

# Model babelforce.v1 in the spec crate

## Goal
Port the frozen payload catalog into `rtvbp-spec-babelforce-v1` as typed Rust — every operation,
every event, their roles, and canonical examples — so it can become the single source of truth for
every SDK and every document.

## Acceptance
- [x] All ten operations are declared with their handling role: `session.initialize` and
      `session.terminate` (application), `session.set`, `session.get`, `application.move`,
      `call.hangup`, `audio.buffer.clear`, `recording.start`, `recording.stop` (voice), and `ping`
      (both).
- [x] All events are declared with their emitting role: `session.updated`, `dtmf`, `call.hangup`,
      `audio.info` (voice); `audio.speech.started`, `output.transcript.delta`,
      `output.transcript.done`, `input.transcript`, `agent.tool.call` (application).
- [x] Field presence uses `T` / `Option<T>` / `Nullable<T>` deliberately per field, matching the
      captured fixtures — notably `SessionInitializeRequest.metadata` and
      `SessionInitializeResponse.audio_codec` are `Nullable`.
- [x] Field declaration order matches the current Go structs (it is the wire's order).
- [x] `application.move`, `call.hangup` and `session.terminate` are flagged `terminal`.
- [x] Every operation and event carries a doc comment and at least one canonical example.
- [x] `catalog()` validates: unique names, every operation and event has a role, every example
      round-trips its own schema.

## Progress
- 2026-08-03: Started; inventorying the frozen Go payloads and adding failing-first catalog
  completeness, presence, role, and validation tests.
- 2026-08-03: Added all typed payloads, roles, terminal flags, documentation, and canonical
  examples; deployed examples serialize byte-for-byte to the frozen payload and event fixtures.
- 2026-08-03: Extended the shared model with typed example round-trips and aggregated catalog
  validation. Tests also pin the legacy Go `float64` zero encoding (`0`, not serde's `0.0`).

## Notes
- Port field-for-field from `rtvbp-go/proto/protov1/*.go`. Prose for doc comments can be lifted from
  `website/docs/protov1/` and the existing Rust port at
  `private-source.invalid/crates/rtvbp/src/protov1.rs`.
- Go type hints where the current API is not `int64`: `AudioCodec.sample_rate` is Go `int`, whereas
  `DtmfEvent.pressed_at` / `released_at` are `int64`. Use the namespaced `x-go-type` extension.
- `ping` is an ordinary catalog operation — do not lift it to a framework concern.
- `session.get` returns a bare open map as its `result`; model it as-is.
- `SessionUpdatedEvent.audio_codec` is also `Nullable<AudioCodec>` because the Go pointer field has
  no `omitempty`. All five legacy Go `int` fields carry `x-go-type: int` schema hints.
