...
## Acceptance
- [x] `rtvbp-go` is imported under `sdk/go/` **with history preserved** (`git subtree` or
      `git-filter-repo`); `git log -- sdk/go` shows the original commits.
- [x] The imported code still builds and its existing tests still pass, untouched — it is the
      reference implementation until the runtime is rewritten.
- [x] `spec/` is a Rust workspace containing `rtvbp-spec-model`, compiling with `cargo test`.
- [x] `rtvbp-spec-model` defines `Catalog`, `Operation`, `Event`, `Role`, `TypeRef`, `Nullable<T>`
      and `EnvelopeSpec`, plus a `classic.v1` reference codec in Rust with unit tests.
- [x] `Nullable<T>` serializes as `null` when empty and marks its schema so emitters can distinguish
      it from `Option<T>` (a unit test pins both behaviours).
- [x] Root `.gitignore` covers Rust and Go build output.

## Progress
- 2026-08-03: Started; importing the pinned `rtvbp-go v0.40.0` history and scaffolding the Rust
  specification model.
- 2026-08-03: Imported the exact `v0.40.0` tree under `sdk/go/`; all 139 rewritten upstream commits
  remain visible through `git log -- sdk/go`.
- 2026-08-03: Added the Rust workspace, catalog model, presence-aware `Nullable<T>`, declarative
  envelope model, and byte-exact `classic.v1` reference codec; all acceptance checks pass.
...
- The untouched legacy `go.work` includes example modules with location-dependent `replace`
  directives. Test the imported root module with `GOWORK=off go test ./...` until the examples move
  in R-12.
