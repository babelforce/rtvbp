...
status: done
priority: 40
design: docs/designs/interactive-browser-protocol-lab.md
...
## Acceptance
- [x] The docs contain an accessible, responsive browser phone with call, mute, DTMF, barge-in,
      clear-buffer, and hangup controls; autoplay, microphone permission, and cleanup states are
      explicit and recoverable.
- [x] A default simulation runs entirely in the browser with no account, credential, private service,
...
- [x] The visualization correlates profile negotiation, session lifecycle, typed requests/responses/
...
- [x] Audio meters or waveforms prove non-silent duplex flow. WebRTC mode shows selected codec,
...
- [x] An optional live mode accepts an endpoint and caller-provided authentication callback/config at
...
- [x] The lab can replay generated conformance scenarios step-by-step and link each visible message to
      its generated reference page, making the specification-to-SDK relationship inspectable.
- [x] Real-browser tests cover the deterministic simulation, fake-device duplex audio, interaction,
...
- [x] A short architecture note defines the trust boundary, sanitization policy, resource ownership,
...
- 2026-08-14: Started implementation after confirming that the published site exposed only static
  browser snippets. The lab will be a first-class `/try` page linked from the landing-page
  **Try it out** call to action.
- 2026-08-14: Completed the glass-box phone with actual TypeScript SDK sessions and generated APIs,
  local WebRTC and WebSocket media proof, browser-reported statistics, generated scenario replay,
  and a fail-closed caller-configured live mode. A failing-first Chrome test now covers the landing
  route, duplex audio, controls, repeated calls, active-call navigation cleanup, and responsive
  layout; the ordinary typecheck and static build remain green.
