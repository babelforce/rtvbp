...
export interface BrowserWebSocketConfig {
  ...
  /** Only injected non-browser socket implementations can apply Upgrade headers. */
...
function browserSocket(config: BrowserWebSocketConfig): WebSocket {
  ...
  if (config.createWebSocket !== undefined) {
    return config.createWebSocket(
      config.url,
      protocols.length === 0 ? undefined : protocols,
      config.headers,
    );
  }
  if (config.headers !== undefined && Object.keys(config.headers).length > 0) {
    throw new SessionError(
      ...
      "native browser WebSocket cannot set Upgrade headers; use subprotocol, query, or cookie deployment policy",
    );
  }
...
/** Open a semantic browser WebSocket transport for a simple or composite binding. */
export async function openBrowserWebSocket(
  ...
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
    throw new SessionError("configuration", "WebSocket connect timeout must be positive");
  }
  if (!Number.isSafeInteger(highWaterMarkBytes) || highWaterMarkBytes <= 0) {
    throw new SessionError("configuration", "WebSocket high-water mark must be positive");
  }
...
export function browserWebSocketTransport(config: BrowserWebSocketConfig): TransportFactory {
  return async (_envelope, signal) => await openBrowserWebSocket(config, signal);
}
